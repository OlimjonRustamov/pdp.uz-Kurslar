...
class DbHelper(context: Context) : SQLiteOpenHelper(context, "pdpuz_kurslar", null, 1), DbMethods {
    override fun onCreate(db: SQLiteDatabase?) {
        val create_barcha_kurslar =
            "CREATE TABLE `barcha_kurslar` (`id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,`kurs_nomi` TEXT NOT NULL,`kurs_haqida` TEXT NOT NULL)"
        val create_mentorlar =
            "CREATE TABLE `mentorlar` (`id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,`mentor_nomi` TEXT NOT NULL,`mentor_familyasi` TEXT NOT NULL,`mentor_otasining_ismi` TEXT NOT NULL, `kurs_id` INTEGER NOT NULL,FOREIGN KEY(`kurs_id`) REFERENCES `barcha_kurslar`(`id`))"
        val create_guruhlar =
            "CREATE TABLE `guruhlar` (`id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,`guruh_nomi` TEXT NOT NULL,`mentor_id` INTEGER NOT NULL,`ochilganligi` TEXT NOT NULL,`kurs_id` INTEGER NOT NULL,`dars_vaqti` TEXT NOT NULL,FOREIGN KEY(`kurs_id`) REFERENCES `barcha_kurslar`(`id`),FOREIGN KEY(`mentor_id`) REFERENCES `mentorlar`(`id`))"
    ...
    override fun insertKurs(kurs: Kurs): Long {
        val contentValues = ContentValues()
        val db = this.writableDatabase
        contentValues.put("kurs_nomi", kurs.kurs_nomi)
        contentValues.put("kurs_haqida", kurs.kurs_haqida)
        return db.insert("barcha_kurslar", null, contentValues)
    }
    ...
    override fun getAllKurs(): ArrayList<Kurs> {
        ...
        if (cursor.moveToFirst()) {
            do {
                kursList.add(Kurs(cursor.getInt(0), cursor.getString(1), cursor.getString(2)))
    ...
    override fun insertMentor(mentor: Mentor): Long {
        val db = this.writableDatabase
        val contentValues = ContentValues()
        ...
        return db.insert("mentorlar", null, contentValues)
    }
    ...
    override fun insertGuruh(guruh: Guruh): Long {
        ...
        return db.insert("guruhlar", null, contentValues)
    }
    ...
    override fun getAllGroupByStatus(ochilganligi: Int): ArrayList<Guruh> {
        val db = this.readableDatabase
    ...
    override fun insertTalaba(talaba: Talaba): Long {
        val db = this.writableDatabase
        val contentValues = ContentValues()
        ...
        return db.insert("talabalar", null, contentValues)
    }
